...
func updateKittyTheme() {
  let kittyApps = NSRunningApplication.runningApplications(
  ...
  if kittyApps.count > 0 {
    ...
    let kittyExecutable = "/Applications/kitty.app/Contents/MacOS/kitty"
    print("Using the executable:", kittyExecutable)

    shell(
      kittyExecutable, args: "+kitten", "themes", "--reload-in=all",
      isDark ? "Gruvbox Dark" : "Gruvbox Light")
  }
}
