...
-- What's the difference between `:packadd` and `:packadd!`(bang)? {{{
--
-- The former will search for an optional plugin in the `packpath` and, if
-- found, add it to the runtimepath and source the plugin files while the latter
-- will only add the directories to the runtime path.
--
